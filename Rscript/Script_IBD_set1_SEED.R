...
###Functions modifying the original astro method
###There are two main ones, annotation (meant as a way to prepare the required files for the method itself, "astro") and astro. 
###Astro requires 3 types of file, node.gwas (the GWAS hits, they are the output file from anotation function),edge.tring (the interactome)
...
###Here comes the fun, the script is meant to allow to loop from each gene

setwd("/nfs/leia/research/beltrao/ibarrioh/IBD/CD")

###Para transferir a yoda

all.gene.gwas=	readRDS("/Network_expansion/tables_IBD/set1/all_gene_gwas_FILTER_set1.rds")
...
############################################################################################################
######MAtrix assembly and final tables
############################################################################################################
############################################################################################################
...
path=paste(	"/Network_expansion/tables_IBD/set1/RDS_astro/",
            "ZSCO.",
            names(dis)[i],
            ".rds",	sep="")
